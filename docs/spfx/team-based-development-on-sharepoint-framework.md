...
Ein SharePoint Framework-Paket ist wie ein Node.js-Paket strukturiert. Alle zugehörigen Abhängigkeiten und Metadaten werden in der Datei **package.json** im Projektordner gespeichert. Eine der Eigenschaften in der Datei **package.json** ist die Version-Eigenschaft, die die Version des gesamten Projekts bezeichnet. Standardmäßig erben alle Komponenten in der aktuellen Lösung diese Versionsnummer als ihre Version. Entwickler sollten die Versionsnummer in der Datei **package.json** jedes Mal erhöhen, wenn eine neue Version des Projekts gemäß der SemVer-Konvention geplant ist.
...
Wenn Sie Ihr Projekt auf eine neuere Version von SharePoint-Framework aktualisieren, sind möglicherweise Änderungen an der Projektstruktur und den Projektkonfigurationsdateien erforderlich. Bevor Sie die Versionen von SharePoint-Framework in Ihren Projektabhängigkeiten aktualisieren, sollten Sie ein neues Projekts mit der Version von SharePoint-Framework erstellen, auf die Sie aktualisieren möchten, und sorgfältig Struktur und Inhalt mit dem bereits vorhandenen Projekt vergleichen. Dadurch können Sie die Auswirkungen der Aktualisierung für Ihr Projekt bestimmten und grundlegende Änderungen an Ihrem Projekt vermeiden.

#### <a name="beware-of-using-npm-outdated"></a>Vorsicht vor der Verwendung von „npm outdated“

Eine Möglichkeiten, um herauszufinden, welche Abhängigkeiten in Ihrem Projekt aktualisiert werden müssen, ist die Ausführung des **npm outdated**-Befehls. Mit diesem Befehl werden Ihre Abhängigkeitsstruktur geprüft und die Pakete angezeigt, die aktualisiert werden können. Obwohl die Verwendung dieses Befehls komfortabel ist, sollte diese Methode mit Vorsicht verwendet werden.

Ab Version 1.3 können Sie im Yeoman-Generator des SharePoint-Framework festlegen, ob ein Projektgerüst, das Sie erstellen möchten, nur in Share Point Online oder sowohl in SharePoint 2016 Feature Pack 2 als auch in SharePoint Online funktionieren soll. Lokal gehostetes SharePoint verwendet eine ältere Version von SharePoint-Framework als die aktuelle Version, die in SharePoint Online verfügbar ist. Wenn Sie den **npm outdated**-Befehl für ein Projekt ausführen, das mit lokalem SharePoint kompatibel ist, würde dieser vorschlagen, alle wichtigen SharePoint-Framework-Pakete auf die für npm veröffentlichte aktuelle Version zu aktualisieren. Leider würde nach der Aktualisierung dieser Pakete auf die neuesten Versionen Ihr Projekt nicht mehr für lokales SharePoint funktionieren. Vor der Aktualisierung der Versionen von SharePoint-Framework-Paketen sollten Sie immer prüfen, ob Ihr Projekt mit lokal gehostetem SharePoint verwendet werden soll, und wenn dies der Fall ist, welche Version von SharePoint-Framework unterstützt werden muss.
