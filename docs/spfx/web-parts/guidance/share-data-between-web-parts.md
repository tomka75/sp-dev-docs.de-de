...
Wenn Sie bei der Erstellung von clientseitigen Webparts Daten nur einmal laden und anschließend in den verschiedenen Webparts jeweils wiederverwenden, verbessert das die Leistung Ihrer Seiten und reduziert die Last in Ihrem Netzwerk. In diesem Artikel stellen wir Ihnen eine Reihe von Methoden vor, mit denen Sie Daten für mehrere Webparts gemeinsam verwenden können.

## <a name="why-sharing-data-between-web-parts"></a>Vorteile der gemeinsamen Verwendung von Daten zwischen Webparts

Bei Webpartprojekten ist es häufig der Fall, dass auf einer einzigen Seite mehrere Webparts eingesetzt werden sollen. Wenn Sie dabei jedes Webpart als unabhängigen Teil der Seite behandeln, laden Sie am Ende einander ähnliche Datensätze oder sogar ein und denselben Datensatz mehrmals auf ein und derselben Seite. Das verlangsamt das Laden der Seite unnötig und verursacht mehr Datenverkehr in Ihrem Netzwerk.
...
Sie können Ihre Webparts so aufbauen, dass sie die Daten nur ein einziges Mal laden. Das beschleunigt das Laden der Seite und reduziert den Datenverkehr in Ihrem Netzwerk. Wann immer eines der Webparts auf der Seite einen spezifischen Datensatz anfordert, verwendet es wenn möglich die zuvor geladenen Daten wieder.
...
> Hinweis. Variablen mit globaler Bereichsdefinition sind in der Regel keine gute Idee. Aber hier werden sie zum Zweck der Illustration und der Einfachheit halber als „Democode“ verwendet. Es gibt viele Muster zu diesem Problem, einschließlich Im- und Exportmodule mit TypeScript-Konzepten.

Webparts, die mit SharePoint Framework erstellt werden, werden in separaten Modulen voneinander isoliert. Daher kann ein Webpart nicht direkt auf Daten und Eigenschaften zugreifen, die von einem anderen Webpart gespeichert werden. Eine Möglichkeit, diese Designstruktur zu umgehen und die von einem Webpart geladenen Daten auch für andere Webparts auf derselben Seite verfügbar zu machen: Sie können die abgerufenen Daten einer Variablen mit globaler Bereichsdefinition zuweisen.

Der oben beschriebene Datenzugriffsdienst müsste dazu wie folgt angepasst werden:
...
Beachten Sie, wie das Laden der Daten aus den spezifischen Methoden in die Methode **ensureRecentDocuments** verlagert wurde. Falls die Daten bereits vorher geladen wurden, löst die Methode die Zusage auf und gibt sofort die zuvor geladenen Dokumente zurück. Werden die Daten gerade erst geladen, wartet die Methode 100 ms lang und versucht dann erneut, die Zusage aufzulösen.
...
Eine weitere Möglichkeit, Daten zwischen unterschiedlichen Webparts auszutauschen, besteht darin, sie in einem Cookie zu speichern. Das hat den zusätzlichen Vorteil, dass die Daten über einen längeren Zeitraum verfügbar sind. Wenn die Daten also selten geändert werden, können Sie sie ein einziges Mal laden und dann sowohl in den verschiedenen Webparts wiederverwenden als auch auf verschiedenen Seiten innerhalb des Portals.

Die Datenspeicherung in einem Cookie lässt sich fast genauso implementieren wie die Speicherung in einer Variablen mit globaler Bereichsdefinition. Dass die eigentlichen Daten in einem Cookie gespeichert werden, ist im Grunde genommen der einzige Unterschied.
...
Bei nachfolgenden Anforderungen könnte das Webpart die zuvor geladenen Daten direkt wiederverwenden, ohne zuerst die Remote-API aufrufen zu müssen.
...
Der in den oben beschriebenen Beispielen demonstrierte Dienst lässt sich mit einigen wenigen Modifizierungen in einen SharePoint Framework-Dienst umwandeln.
